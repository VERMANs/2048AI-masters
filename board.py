...
class Board(object):
	...
	def __init__(self, board=[[0]*4]*4, score=0, goal=2048):
		# all initialized within setBoardParams
		self.size = None
		self.rows = None
		self.columns = None
		self.board = None
		if self.validBoard(board):
			self.setBoardParams(board)
		else:
			sys.exit(1)
		...
		self.goal = None
		self.setWinGoal(goal)

		self.__won = False

		self.initState()
		# [highest_tile, row, column]
		self.__highest = self.highestTile()
	...
	def validBoard(self, board):
		...
		lc = len(board)
		for row in board:
			if len(row) != lr or lr != lc:
				return False
		return True

	def setBoardParams(self, board):
		'''sets the game board according to the board configurations passed to it'''
		self.board = board
		self.size = len(board)
		self.rows = len(board)
	...
	def setWinGoal(self, goal):
		'''sets the tile required to win the game'''
		self.goal = goal
	...
	def getEmptyCells(self):
		b = self.board
		return len([b[i][j] for i in range(self.size) for j in range(self.size) if b[i][j] == 0])
	...
	def move(self, dir):
		...
		if self.highestTile()[0] == self.goal:
			self.__won == True
